...
def _check_schema_version(path: Path, *, expected: int | None = None) -> None:
    """Raise ValueError if the parquet file's schema_version doesn't match the expected version."""
    if expected is None:
    ...
    if raw is None:
        raise ValueError(
            ...
            f"expected {expected}. Rebuild the panel with surg-prep."
        )
    file_version = raw.decode()
    if file_version != str(expected):
        raise ValueError(
            f"schema_version mismatch in {path}: file has {file_version!r}, "
            f"code expects {str(expected)!r}. Rebuild the panel with surg-prep."
        )
